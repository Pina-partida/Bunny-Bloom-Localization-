...
function endGame()
{
	...
	var msg = _("Number of bunnies blooming:");
	
	switch (randomMessage)
	{
		case 1:
			msg = _("Number of bunnies flowering:");
			break;
		case 2:
			msg = _("Number of bunnies blossoming:");
			break;
		case 3:
			msg = _("Number of bunnies sprouting:");
			break;
		case 4:
			msg = _("Number of bunnies flourishing:");
			break;
	}
	
	message.innerHTML = msg + '&nbsp' + scoreP.firstChild.nodeValue;
	message.innerHTML += ('<input class="button" id="shotgunButton" type="button" value= "' + _('reload your flower power!') + '" onClick="reinitialise()">');
	message.innerHTML += ('<input class="button" id="stopButton" type="button" value=  "' + _('Time to take a nap') + '" onClick="stop()">');
...
function stop()
{
	...
	closing.innerHTML += '<p class="closingMessage">' + _('The bunnies of the world thank you for your pacifism.') + '</p>';
...
function levelUp()
{
	...
	levelMessage.innerHTML = "Nivel " + level;
			...
			if (clickX >= position[0] && clickX <= position[0] + bunny.offsetWidth)
			{
				if (clickY >= position[1] && clickY <= position[1] + bunny.offsetHeight)
				{
					...
					else
					{
						...
						var score = document.getElementById("score");
						var scoreP = score.getElementsByTagName("p")[0];
						scoreP.firstChild.nodeValue = new Intl.NumberFormat(locale, { style: "decimal" }).format(parseInt(scoreP.firstChild.nodeValue) + 1);
					}
